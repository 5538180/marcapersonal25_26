...
class CicloFormativoSeeder extends Seeder
{
    ...
    {
        if (FamiliaProfesional::query()->doesntExist()) {
        ...
        foreach (self::$ciclos as $ciclo) {
            $grado = self::mapGrado($ciclo['grado']);
            $codigoCiclo = $ciclo['codCiclo'];

            if ($grado === null || isset($codigosSembrados[$codigoCiclo])) {
                continue;
            }

            $familiaProfesional = FamiliaProfesional::query()
                ->where('codigo', self::mapCodigoFamilia($ciclo['codFamilia']))
                ->firstOrFail();

            CicloFormativo::query()->updateOrCreate(
                ['codigo' => $codigoCiclo],
                [
                    'familia_profesional_id' => $familiaProfesional->id,
                    'nombre' => $ciclo['nombre'],
                    'grado' => $grado,
                    'slug' => Str::slug($codigoCiclo.'-'.$ciclo['nombre']),
                ],
            );

            $codigosSembrados[$codigoCiclo] = true;
        }

        $this->command->info('¡Tabla ciclos formativos inicializada con datos!');
    }

    private static function mapGrado(string $grado): ?string
    {
        return [
            'G.M.' => 'medio',
            'G.S.' => 'superior',
    ...
    private static function mapCodigoFamilia(string $codigo): string
    {
        return [
            'ADG' => 'AFD',
            'AFD' => 'ADG',
